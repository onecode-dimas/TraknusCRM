...
namespace Check_Approval_TOP
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            try
            {
                ...
                foreach (var entity in QP_Headers.Entities)
                {
                    ...
                    try
                    {
                        if (entity.Contains("tss_topcurrentapprover") && entity.Contains("tss_topdatetime") && entity.Contains("tss_branch"))
                        {
                            DateTime currentdatetime = DateTime.Now;
                            DateTime topdatetime = entity.GetAttributeValue<DateTime>("tss_topdatetime").ToLocalTime();
                            EntityReference branch = entity.GetAttributeValue<EntityReference>("tss_branch");
                            ...
                            QueryExpression queryMatrixapprovaltop = new QueryExpression("tss_matrixapprovaltop")
                            {
                                ...
                                Orders =
                                {
                                   new OrderExpression("tss_priorityno", OrderType.Descending)
                                }
                            };
                            ...
                            if (currPriorty != 1)
                            {
                                ...
                                if (nextApprover != null)
                                {
                                    ...
                                    SendEmailNotif(entity.Id, nextApproval.Id, _orgService);
                        ...
                        {
                            Console.WriteLine("Field top current approver, top date or branch is null");
                        }
                    }
                    catch (Exception ex)
                    {
        ...
        public static void SendEmailNotif(Guid quotationID, Guid receiver, IOrganizationService organizationService)
        {
            try
            {
                ...
                string quotationId = quotationheader.GetAttributeValue<string>("tss_quotationid");

                var emailGuid = CreateEmailNotif(crmadmin, receiver, crmadmin,
                    organizationService, quotationId, quotationheader.Id.ToString());
        ...
        private static Guid CreateEmailNotif(Guid senderGuid, Guid receiverGuid, Guid ccGuid, IOrganizationService organizationService, string Topic, string recordID)
        {
            try
            {
                ...
                var subject = @"Waiting Approval change TOP";
                string row = string.Empty;
                var bodyTemplate = @"Dear Mr/Ms " + Fullname + @",<br/>
                                Please approve the change top in Quotation Part, with QuotationId : <a href='http://10.0.10.43/TraktorNusantara/main.aspx?etc=10184&id=%7b" + recordID + "%7d&pagetype=entityrecord'>" + Topic + "<a>";
