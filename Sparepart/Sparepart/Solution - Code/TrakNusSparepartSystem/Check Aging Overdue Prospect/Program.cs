    ...
    class Program
    {
        ...
        public static void SendEmailNotif(Guid prospectheaderID, IOrganizationService organizationService)
        {
            try
            {
                ...
                if (prospectheader.GetAttributeValue<EntityReference>("tss_pss") != null)
                {
                    //Get PSS of prospect
                    pssId = prospectheader.GetAttributeValue<EntityReference>("tss_pss").Id;
                    Entity userTssEntity = organizationService.Retrieve("systemuser", pssId, new ColumnSet(true));

                    if (userTssEntity.GetAttributeValue<EntityReference>("parentsystemuserid") != null)
                    {   
                        //Get PSS's manager
                        tssManagerId = userTssEntity.GetAttributeValue<EntityReference>("parentsystemuserid").Id;
                    }
                }
                ...
                Guid[] Receiver = new Guid[receivers.Entities.Count + 1];
                ...
                Receiver[x] = tssManagerId;

                string topic = prospectheader.GetAttributeValue<string>("tss_topic");
                ...
                var emailGuid = CreateEmailNotif(crmadmin, pssId, Receiver, crmadmin,
        ...
        private static Guid CreateEmailNotif(Guid senderGuid, Guid receiverGuid, Guid[] receiverCCGuids, Guid ccGuid, IOrganizationService organizationService, string Topic, string Description)
        {
            try
            {
                ...
                {
                    emailAgent.AddCC("systemuser", receiverCC); //bermasalah owning usernya
                }
