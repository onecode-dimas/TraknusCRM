...
{
    public class BL_MarketSizeResultNational
    {
        #region Dependencies
        ...
        public void checkEscalationApprovalforMarketSizeResultNational()
        {
            try
            {
                ...
                #region get Market Size Result National
                ...
                fMSP.AddCondition("tss_status", ConditionOperator.Equal, 865920001);

                QueryExpression qMSP = new QueryExpression("tss_marketsizeresultnational");
                qMSP.Criteria.AddFilter(fMSP);
                qMSP.ColumnSet = new ColumnSet(true);

                EntityCollection ENC_MarketSizeResultNational = _orgService.RetrieveMultiple(qMSP);

                #endregion

                #region check if Approved
                foreach (Entity _entity in ENC_MarketSizeResultNational.Entities)
                {
            ...
            {
                Console.WriteLine("Unexpected error while checking Market Size Result National Escalation. Error Details : " + ex.Message.ToString());
            }
        }
