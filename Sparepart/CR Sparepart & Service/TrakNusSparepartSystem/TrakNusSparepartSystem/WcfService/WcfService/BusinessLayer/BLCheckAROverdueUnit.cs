...
{
    public class BLCheckAROverdueUnit : ICheckAROverdue
    {
        public WS_Response_CheckAROverdue CheckAR(string transNumber, string customerCode, string businessTransType, IOrganizationService serviceProxy)
        {
            ...
            int AROverdue = 0, AROverdueService = 0, AROverduePart=0;
            ...
            string fetchHeader = String.Format(@"
                            <fetch version='1.0' output-format='xml-platform' mapping='logical' distinct='false'>
                              <entity name='salesorder'>
                                ...
                                <filter type='and'>
                                  <condition attribute='new_cpoidsap' operator='eq' value='{0}' />
                                </filter>
                              </entity>
                            </fetch>", transNumber);
            ...
            {
                refAccount = c.Attributes.Contains("customerid") ? c.GetAttributeValue<EntityReference>("customerid") : new EntityReference();
                totalBilling = c.Attributes.Contains("new_totalamount") ? c.GetAttributeValue<Money>("new_totalamount").Value : 0;
            ...
            {
                AROverdue = c.Attributes.Contains("op.ibizcs_overduedaysarunit") ? (int)c.GetAttributeValue<AliasedValue>("op.ibizcs_overduedaysarunit").Value : 0;
                ...
                AROverduePart= c.Attributes.Contains("op.ibizcs_overduedaysarparts") ? (int)c.GetAttributeValue<AliasedValue>("op.ibizcs_overduedaysarparts").Value : 0;
                CurrenAR = c.Attributes.Contains("op.ibizcs_currentarunit") ? ((Money)c.GetAttributeValue<AliasedValue>("op.ibizcs_currentarunit").Value).Value : 0;
                OverdueAmountAR = c.Attributes.Contains("op.ibizcs_overdueamountarunit") ? ((Money)c.GetAttributeValue<AliasedValue>("op.ibizcs_overdueamountarunit").Value).Value : 0;
                plafond = c.Attributes.Contains("op.ibizcs_plafonarunit") ? ((Money)c.GetAttributeValue<AliasedValue>("op.ibizcs_plafonarunit").Value).Value : 0;
            ...
            else if (AROverdue > 0 || AROverdueService > 0||AROverduePart>0)
            {
                response.Flag = false;
                response.Message = string.Format("AR {0} OD for Customer Code {1}, {2} days", AROverdue > 0 ? "Unit" : AROverdueService > 0 ? "Service" : "Part", customerCode, AROverdue > 0 ? AROverdue : AROverdueService > 0 ? AROverdueService : AROverduePart);
            }
            else
            {
                ...
                response.Message = string.Format("Remaining Plafond Unit for Customer Code {0}, Rp. {1}", customerCode, remainingPlafond.ToString("N0"));
            }
            return response;
