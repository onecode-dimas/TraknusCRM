...
namespace TrakNusSparepartSystemWcfService
{
    ...
    public class Service : IService
    {
        ...
        public WS_Response_CheckAROverdue CheckAROverdue(string token, string TransNumber, string CustomerCode, string BusinessTransType)
        {
            WS_Response_CheckAROverdue response = new WS_Response_CheckAROverdue();
            try
            {
                string _connectionString = string.Empty;
                ICheckAROverdue blChekAROverdue = null;
                if (IsValidConnectionString(ConfigurationManager.ConnectionStrings["TRSCRM"].ConnectionString))
                    _connectionString = ConfigurationManager.ConnectionStrings["TRSCRM"].ConnectionString;
                ...
                using (CrmServiceClient conn = new CrmServiceClient(_connectionString))
                {
                    ...
                    string generateToken = Encrypt(TransNumber.ToString(), uniqueKey);
                    if (generateToken == token)
                    {
                        if (BusinessTransType == BusinessTransactionType.Service.ToString().ToUpper())
                            blChekAROverdue = new BLChecckAROverdueService();
                        else if (BusinessTransType == BusinessTransactionType.Sparepart.ToString().ToUpper())
                            blChekAROverdue = new BLCheckAROverduePart();
                        else if(BusinessTransType==BusinessTransactionType.Unit.ToString().ToUpper())
                            blChekAROverdue = new BLCheckAROverdueUnit();
                        else
                        {
                            response.Flag = false;
                            response.Message = "Invalid Business Transaction Type";
                        }
                        
                        if(blChekAROverdue!=null)
                            response = blChekAROverdue.CheckAR(TransNumber, CustomerCode, BusinessTransType, organizationService);
                    }
                    else
                    {
                        throw new Exception("Access denied invalid token!");
            ...
            {
                response.Flag = false;
                response.Message = "Global error  : " + ex.Message;
                _mwsLog.Write(MethodBase.GetCurrentMethod().Name, "Global Error" + _mwsLog.ColumnsSeparator
                                 + "Error : " + ex.Message.ToString(), MWSLog.LogType.Error, MWSLog.Source.Inbound);
            }
            return response;
        }
