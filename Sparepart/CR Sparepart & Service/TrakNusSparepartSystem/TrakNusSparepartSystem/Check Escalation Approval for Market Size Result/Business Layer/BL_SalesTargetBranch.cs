...
{
    public class BL_SalesTargetBranch
    {
       
        private const int APPKORWIL = 865920001;
        static DL_tss_salestargetbranch _DL_tss_salestargetbranch = new DL_tss_salestargetbranch();
        ...
        public void checkEscalationApprovalforSalesTargetBranch()
        {
            ...
            CrmServiceClient conn = new CrmServiceClient(_connectionString); try
            {
                ...
                fBranch.AddCondition("tss_status", ConditionOperator.Equal, APPKORWIL);

                QueryExpression qBranch = new QueryExpression(_DL_tss_salestargetbranch.EntityName);
                qBranch.Criteria.AddFilter(fBranch);
                qBranch.ColumnSet = new ColumnSet(true);
                EntityCollection stBranch = _DL_tss_salestargetbranch.Select(_orgService, qBranch);

                if (stBranch.Entities.Count > 0)
                {
                    foreach (var branch in stBranch.Entities)
                    {
                        FilterExpression fMat = new FilterExpression(LogicalOperator.And);
                        fMat.AddCondition("tss_branch", ConditionOperator.Equal, branch.GetAttributeValue<EntityReference>("tss_branch").Id);
                        fMat.AddCondition("tss_approvaltype", ConditionOperator.Equal, 865920001);/* approval type KORWIL*/
                        ...
                        Entity currMats = mats.Entities.Where(o => o.Id == branch.GetAttributeValue<EntityReference>("tss_currentapproval").Id).FirstOrDefault();

                        DateTime currApprovalDate = branch.GetAttributeValue<DateTime>("tss_currentapprovaldate");
                        ...
                        if (mats.Entities.Count > 0)
                        {
                            if (diff > currEscHr)
                            {
                                ...
                                if (nextApp != null)
                                {
                                    ...
                                    string emailSubject = branch.GetAttributeValue<string>("tss_name") + " is waiting for KORWIL approval";
                                    ...
                                    branch["tss_currentapproval"] = nextApp.GetAttributeValue<EntityReference>("tss_approver").Id;
