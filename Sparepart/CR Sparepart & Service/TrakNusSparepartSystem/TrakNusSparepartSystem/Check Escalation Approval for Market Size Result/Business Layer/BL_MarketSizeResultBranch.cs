...
{
    public class BL_MarketSizeResultBranch
    {
        private const int APPKORWIL = 865920001;
        static DL_tss_marketsizeresultbranch _DL_tss_marketsizeresultbranch = new DL_tss_marketsizeresultbranch();
        ...
        public void checkEscalationApprovalforMarketSizeResultPSS()
        {
            ...
            CrmServiceClient conn = new CrmServiceClient(_connectionString); try
            {
                ...
                EntityCollection msBranch = _DL_tss_marketsizeresultbranch.Select(_orgService, qBranch);

                if (msBranch.Entities.Count > 0)
                {
                    foreach (var branch in msBranch.Entities)
                    {
                        ...
                        if (mats.Entities.Count > 0)
                        {
                            if (diff > currEscHr)
                            {
                                ...
                                if (nextApp != null)
                                {
                                    ...
                                    _DL_tss_marketsizeresultbranch.tss_currentapproval = nextApp.GetAttributeValue<EntityReference>("tss_approver").Id;
                                    _DL_tss_marketsizeresultbranch.Update(_orgService, branch.Id);
